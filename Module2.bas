Public Function ObtenerFilaInicioDesdeHoja(ByVal yearBuscado As Long) As Long
    Dim ws As Worksheet
    Dim primerYear As Long
    ...
    primerYear = ws.Cells(2, 1).value
    If Not IsNumeric(primerYear) Or primerYear = 0 Then Exit Function

    filaBase = 2
    posRel = yearBuscado - primerYear
    ObtenerFilaInicioDesdeHoja = filaBase + posRel * 13
End Function
...
Private Function EsTokenNumerico(ByVal token As String) As Boolean
    Dim clean As String, i As Long, ch As String, hasDigit As Boolean
    clean = Trim$(token)
    clean = Replace(clean, "$", "")
    clean = Replace(clean, "(", "")
    clean = Replace(clean, ")", "")
    clean = Replace(clean, Chr$(160), "")
    clean = Replace(clean, ".", "")
    clean = Replace(clean, ",", "")
    clean = Replace(clean, "-", "")
                ...
                If BuscarEnArray(Trim$(Lineas(t))) Then
                    ...
                    For k = scanEnd To t + 1 Step -1
                        If Len(Trim$(Lineas(k))) > 0 Then
                            ' Solo aceptamos token numérico; si no existe, se deja vacío (sin fallback de texto)
                    ...
                    Else
                        ' fallback: ?ltimo token no vac?o (por si tiene digits mezclados)
                        Dim último As String
                        último = LastNonEmptyToken(Lineas)
                        valorToken = Trim$(último)
                    End If
...
Function FormatearNumero(ByVal Valor As String) As String
    ...
    Valor = Replace(Valor, "(", "")
    Valor = Replace(Valor, ")", "")
    Valor = Replace(Valor, Chr$(160), "")
    
    ' Extraer solo los dígitos (y signo) si hay texto mezclado
    For i = 1 To Len(Valor)
        ch = Mid$(Valor, i, 1)
        If ch >= "0" And ch <= "9" Then
            soloDigitos = soloDigitos & ch
        ElseIf ch = "-" And soloDigitos = "" Then
            soloDigitos = ch
        End If
    Next i
    If Len(soloDigitos) > 0 Then Valor = soloDigitos
...
"732", "734", "735", "738", "739", "740", "741", "742", "743", "744", "745", "749", "750", "751", "755", "756", "758", "759", "761", _
...
Sub LeerArchivos(ByVal yearInput As Long, ByVal tipo As Integer, ByVal posicion As Integer)
  ...
  Call DeterminarRangoAnios("Ventas", 3)
  Call DeterminarRangoAnios("Compras", 4)
  Call DeterminarRangoAnios("Ventas", 5)
...
End Sub
Sub DeterminarRangoAnios(NombreHoja As String, Columna As Integer)
    Dim wsSrc As Worksheet
    Dim wsDest As Worksheet
    Dim UltimaFila As Long
    Dim AnoInicio As Long
    Dim AnoFinal As Long
    Dim i As Long
    ...
    For i = 2 To UltimaFila
        AnoActual = wsSrc.Cells(i, 1).value ' Leer el a?o en la columna A
        If wsSrc.Cells(i, Columna).value <> "" Then ' Verificar si hay un archivo en la columna C
            If AnoInicio = 0 Then
                AnoInicio = AnoActual ' Asignar el primer a?o encontrado
            End If
            AnoFinal = AnoActual ' Actualizar el ?ltimo a?o encontrado
        End If
    Next i
     

    ' Asignar los valores directamente en las celdas de la hoja "F29"
    wsDest.Cells(1, 7).value = AnoInicio ' Columna G: A?o de inicio
    wsDest.Cells(1, 9).value = AnoFinal ' Columna I: A?o final
